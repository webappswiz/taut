/*! ┌─────────────────────────────────────────────────────────────────────────────────────────────┐ */
/*! │ RaphBoard v1.3.0 - Cross-browser drawing board based on Raphaël                             │ */
/*! ├─────────────────────────────────────────────────────────────────────────────────────────────┤ */
...
( function ( $, window, document, undefined ) {
	...
	var _buttonIcon = {
		init: function ( button, path, attr ) {
			this._button = button;
			this._path = button._toolBar._paper.path( path ).attr( attr );
		...
		glow: function ( state ) {
			if ( state ) {
				this._path._glow = this._path.glow();
			} else {
				if ( this.isGlowing() ) {
					this._path._glow.remove();
				}
			}
	...
	var _toolBar = {
		init: function ( RB, id ) {
			...
			for ( var name in Tools ) {
				...
				button
					.addIcon( ButtonIcon( button, Tools[name].path, {
						fill		: "90-#888-#CCC",
						stroke		: "none",
		...
		insertButton: function ( button, index ) {
			this._buttons.splice( index, 1, button );
	...
	var _attributesPanel = {
		...
		show: function () {
			var self = this,
				...
				colors = ["#000", "#FFF", "#F00", "#0F0", "#00F", "#FF0"],
			...
			picker = self._paper.circle( w/2, h/2, 90 ).attr({
			...
			var s			= self._paper.set(),
				_fillBack	= self._paper.circle( w/2, ( h/2 ), 16 ).attr({
				fill	: "rgba(0,0,0,0)",
				stroke	: "none"
			});
			s.push( _fillBack );
			...
			s.attr( { cursor: "pointer" } )
				.hover(
					function () {	// mouse_in
						_fillBack.attr( { fill: "rgba(255,255,255,0.5)" } );
					},
					function () {	// mouse_out
						_fillBack.attr( { fill: "rgba(0,0,0,0)" } );
					}
				)
				.click( function () {
			...
			// Cancel Button
			var _cancelBack = self._paper.circle( ( w/2 ) - 90, ( h/2 ) + 90, 16 ).attr({
				fill	: "rgba(0,0,0,0)",
				stroke	: "none"
			});
			s = self._paper.set();
			...
			s.attr( { cursor: "pointer" } )
				.hover(
					function () {	// mouse_in
						_cancelBack.attr({ fill: "rgba(255,0,0,0.25)" });
					},
					function () {	// mouse_out
						_cancelBack.attr({ fill: "rgba(0,0,0,0)" });
					}
				)
				.click( function () {
					self.hide();
			...
			// OK Button
			var _okBack = self._paper.circle( ( w/2 ) + 90, ( h/2 ) + 90, 16 ).attr({
				fill	: "rgba(0,0,0,0)",
				stroke	: "none"
			});
			s = self._paper.set();
			...
			s.attr( { cursor: "pointer" } )
				.hover(
					function () {	// mouse_in
						_okBack.attr({ fill: "rgba(0,255,0,0.25)" });
					},
					function () {	// mouse_out
						_okBack.attr({ fill: "rgba(0,0,0,0)" })
					}
				)
				.click( function () {
	...
	var Board = {
		init: function ( options, elem ) {
			...
			// The Toolbar
			// Only if included!
			if ( typeof _toolBar !== 'undefined' && _toolBar ) {
		...
		initMove: function ( id, hideHandles ) {
			...
			if ( element && element.freeTransform ) {
				ft = element.freeTransform;
			} else {
				ft = InitMove( self, element );
			}
			if ( hideHandles ) {
		...
		freeTransform: function ( id, attrs, callback ) {
			var self	= this,
				element	= self.element( id );

			if ( element ) {
				if ( typeof attrs === "object" ) {
					SetFreeTransformAttrs( self, element, attrs, callback )
				...
				if ( json.hasOwnProperty( i ) ) {
					...
					if ( data ) {
						if ( data.ft && data.ft.attrs ) {
							SetFreeTransformAttrs( self, element, data.ft.attrs, function ( ft, events ) {
								if ( ( events[0] == "apply" || events[0].split(" ")[1] == "end" ) && callback ) {
									// Async Callback
									callback( element );
								}
							});
			...
			if ( self.canUndo() ) {
				if ( EventHandler( self, "before_undo" ) ) {
					...
					switch( mode ) {	// "move|pen|line|arrow|circle|ellipse|rect|text|cut|clear"
						case "move":
							prev = PreviousShapeInBuffer( undoBuffer, shape.element.id );
							if ( prev ) {
			...
			if ( self.canRedo() ) {
				if ( EventHandler( self, "before_redo" ) ) {
					var elements	= self.elements,
						redoBuffer	= self.redoBuffer,
						paper		= self.paper,
						shape		= redoBuffer.pop(),
						mode		= shape.command,
		...
		toggleUndoRedo: function () {
			if ( this.toolBar ) {
				...
				var handler = this.eventHandlers["updateUndoRedo"];

				if ( handler ) {
					handler( this );
				} else {
					this.toolBar.toggleUndoRedo();
				}
			}
	...
	function InitMove( self, element ) {
		var moved = false,
			...
			_hasPropertyValues = function ( object ) {
				// Check if object has property values (other than "null" and "undefined")
				...
				switch ( events[0] ) {	// Single events only
					...
					case "drag start":
						if ( ft.handles.center ) {
							if ( _hasPropertyValues( ft.handles.center.disc._drag ) ) {
								// We're dragging by the center handle
								["bbox", "circle"].map( function ( key ) {
									if ( ft[key] ) {
										ft[key].hide();
									}
								});
								["x", "y"].map( function ( key ) {
									if ( ft.handles[key] ) {
										for ( var elem in ft.handles[key] ) {
											ft.handles[key][elem].hide();
										}
									}
								});
								if ( ft.handles.bbox ) {
									ft.handles.bbox.map( function ( box ) {
										box.element.hide();
									});
								}
							} else {
	...
	function OnMouseDown( e ) {
		...
		if ( EventHandler( self, "before_start" ) ) {
			...
			if ( self._mode != "text" && self._mode != "cut" ) {
				if ( self._mode != "move" || self.multiSelect ) {
	...
	function SetFreeTransformAttrs( self, element, attrs, callback ) {
		var doUnplug = false,
			ft = element.freeTransform,
			oldAnimate = false,
			oldCallback = null,
			newCallback = function ( ft, events ) {
				if ( callback ) {
					callback( ft, events );
				}
				if ( !doUnplug ) {
					...
					ft.callback = oldCallback;
					if ( oldCallback ) {
						oldCallback( ft, events );
					}
				} else {
					ft.unplug();
				}
		...
		if ( !ft ) {
			...
			oldCallback = ft.callback;
		}

		ft.callback = newCallback;

		if ( !oldCallback && self.options.animation !== "" ) {
			oldAnimate = ft.opts.animate;
			ft.setOpts({
				animate: self.options.animation
			});
		} else {
			ft.setOpts({
				animate: false	// Just in case...
			});
		}
