...
( function ( window, document ) {
	...
	var gnMenu = function ( el, options ) {
		this.el = el;
		this._init();
	};
	...
	gnMenu.prototype = {
		...
		_init : function() {
			...
			self.mainMenu = document.getElementById( 'gn-menu' );
			self.menu = self.el.querySelector( 'nav.gn-menu-wrapper' );
			self.trigger = self.el.querySelector( 'a.gn-icon-menu' );
			self.isMenuOpenedPartly = false;
			self.isMenuOpenedFully = false;
			self.eventType = mobileCheck() ? 'touchstart' : 'click';
			self.events = {
				onMainMenuEnter: function( ev ) {
					self._expandMainMenu();
				},
				onMainMenuExit: function( ev ) {
					if ( !( self.isMenuOpenedPartly || self.isMenuOpenedFully ) ) {
						self._shrinkMainMenu();
					}
				},
		...
		_initEvents : function() {
			...
			if ( !mobileCheck() ) {
				self.mainMenu.addEventListener( 'mouseover', self.events.onMainMenuEnter );
				self.mainMenu.addEventListener( 'mouseout', self.events.onMainMenuExit );
		...
		_expandMainMenu : function() {
			classie.add( this.mainMenu, 'fc-menu-expand' );
		},

		_shrinkMainMenu : function() {
			classie.remove( this.mainMenu, 'fc-menu-expand' );
		},
		...
		_openIconMenu : function() {
			...
			$( '#gn-menu .gn-menu' ).each(function(){ $( this ).addClass( 'fc-menu-expand' ); });
		...
		_closeIconMenu : function() {
			...
			$( '#gn-menu .gn-menu' ).each(function(){ $( this ).addClass( 'fc-menu-shrink' ).removeClass( 'fc-menu-expand' ); });

			setTimeout( function () {
				$( '#gn-menu .gn-menu' ).each(function(){ $( this ).removeClass( 'fc-menu-shrink' ); });

				classie.remove( self.menu, 'gn-open-part' );
		...
		open : function( callback ) {
			...
			classie.add( self.menu, 'gn-open-all' );
			self._closeIconMenu();
