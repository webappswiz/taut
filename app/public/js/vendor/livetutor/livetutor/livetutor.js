	...
	LT.prototype = {
		...
		*/
		_videoConnect: function () {
			...
			if ( OT.checkSystemRequirements() ) {
				...
				self.session.on({
					streamCreated      : self._streamCreatedHandler,
					streamDestroyed    : self._streamDestroyedHandler,
					sessionConnected   : self._sessionConnectedHandler,
					sessionDisconnected: self._sessionDisconnectedHandler
				});
		...
		_sessionConnectedHandler: function ( event ) {
			...
			if ( self.session.capabilities.publish ) {
				...
				self.publisher.on({
					accessDialogOpened: self._accessDialogOpenedHandler,
					accessDialogClosed: self._accessDialogClosedHandler,
					accessAllowed     : self._accessAllowedHandler,
					accessDenied      : self._accessDeniedHandler,
					streamCreated     : self._publishedStreamCreatedHandler,
					streamDestroyed   : self._publishedStreamDestroyedHandler
				});
		...
		_subscribeToStream: function ( streamData ) {
			...
			if ( self.onSubscribedToStream ) {
				self.onSubscribedToStream( streamData, self._onSubscribeToStream );
			} else {
		...
		*/
		...
		_clearIdleTimeout: function () {
			...
			clearTimeout( self.idleTimer.bind( self ) );
		},
